...
public class ShopeVirtual {
    ...
    public String searchOrderName(String name) {
        ...
        if (index != -1) {
            ...
            msj = " la orden a nombre de " + name + " tiene un precio de " + foundOrder.getPrice()
                    + " y fue  hecha a las  " + foundOrder.getDateBuy().format(DateTimeFormatter.ofPattern("HH:mm:ss"));
        } else if (index == -1) {
    ...
    public String searchOrderPrice(double price) {
        ...
        if (index != -1) {
            Order foundOrder = orders.get(index);
            msj = "La orden a nombre de " + foundOrder.getNameBuyer() + " tiene un precio de " + price
                    + "  y fue hecha a las  " + foundOrder.getDateBuy().format(DateTimeFormatter.ofPattern("HH:mm:ss"));
    ...
    public String searchOrderTime(LocalTime time) {
        ...
        if (index != -1) {
            Order foundOrder = orders.get(index);
            msj = " la orden a nombre de " + foundOrder.getNameBuyer() + " tiene un precio de " + foundOrder.getPrice()
                    + " y fue hecha a las   " + time.format(DateTimeFormatter.ofPattern("HH:mm:ss"));
        } else if (index == -1) {
    ...
    public String searchRangePrice(int numinf, int nummax, int type) {
        ...
        for (int i = 0; i < resultados.size(); i++) {
            msj += " la orden de precio  " + resultados.get(i).getPrice() + "  a nombre de "
                    + resultados.get(i).getNameBuyer() + "\n";
        }
    ...
    public String searchProductName(String name) {
        ...
        if (index != -1) {
            ...
            msj = " el producto a nombre de " + name + " que tiene como descripcion " + foundProduct.getDescription()
                    + " y con un precio de  " + foundProduct.getPrice() + " ,con una cantidad disponible de "
                    + foundProduct.getAmount();
        } else if (index == -1) {
    ...
    public String searchProductPrice(double price) {
        ...
        if (index != -1) {
            Product foundProduct = products.get(index);
            msj = " el producto a nombre de " + foundProduct.getName() + " que tiene como descripcion "
                    + foundProduct.getDescription() + " y con un precio de  " + price
                    + " ,con una cantidad disponible de " + foundProduct.getAmount();
        } else if (index == -1) {
            System.out.println("producto no encontrado.");
        }
        return msj;
    ...
    public String searchProductCategory(int Categori) {
        ...
        if (index != -1) {
            Product foundProduct = products.get(index);
            msj = " el producto a nombre de " + foundProduct.getName() + " que tiene como descripcion "
                    + foundProduct.getDescription() + " y con un precio de  " + foundProduct.getPrice()
                    + " ,con una cantidad disponible de " + foundProduct.getAmount();
        } else if (index == -1) {
            System.out.println("producto no encontrado.");
        }
    ...
    public String searchRangeProductNumeric(double min, double max, int typeSearch, int typeO) {
        String msj = "";
        int start = 0;
        int end = 0;
        ...
        switch (typeSearch) {
            ...
            case 1:
                ...
                start = search.binsearchabbRangePriceIzP(products, min);
                if (start < 0) {
                    start = -(start + 1);
                }
                end = search.binsearchabbRangePriceDeP(products, max);
                if (end < 0) {
                    end = -(end + 2);
                }
                break;
            case 2:
                ...
                start = search.binsearchabbRangeAmountIzP(products, min);
                if (start < 0) {
                    start = -(start + 1);
                }
                end = search.binsearchabbRangeAmountDeP(products, max);
                if (end < 0) {
                    end = -(end + 2);
                }
            ...
            case 3:
                Collections.sort(products, new Comparator<Product>() {
                    @Override
                    public int compare(Product p1, Product p2) {
                        return (int) (p1.getNumber_bought() - p2.getNumber_bought());
                    }
                });
                start = search.binsearchabbRangeBoughtIzP(products, min);
                if (start < 0) {
                    start = -(start + 1);
                }
                end = search.binsearchabbRangeBoughtDeP(products, max);
                if (end < 0) {
                    end = -(end + 2);
                }
                for (int i = start; i <= end; i++) {
        ...
        switch (typeSearch) {
            case 1:
                if (typeO == 1) {
                    resultados.sort(Comparator.comparing(Product::getPrice).reversed());
                    msj += "Los productos en el rango de mayor a menor son:\n";
                } else {
                    resultados.sort(Comparator.comparing(Product::getPrice));
                    msj += "Las órdenes en el rango de precio de menor a mayor son:\n";

                }
                for (int i = 0; i < resultados.size(); i++) {
                    msj += " el orden es  " + resultados.get(i).getPrice() + "  a nombre de "
                            + resultados.get(i).getName() + "\n";
                }
                break;
            case 2:
                if (typeO == 1) {
                    resultados.sort(Comparator.comparing(Product::getAmount).reversed());
                    msj += "Los productos en el rango de mayor a menor son:\n";
                } else {
                    resultados.sort(Comparator.comparing(Product::getAmount));
                    msj += "Las órdenes en el rango de precio de menor a mayor son:\n";
                }
                for (int i = 0; i < resultados.size(); i++) {
                    msj += " la orden es  " + resultados.get(i).getAmount() + "  a nombre de "
                            + resultados.get(i).getName() + "\n";
                }
                break;
            case 3:
                if (typeO == 1) {
                    resultados.sort(Comparator.comparing(Product::getNumber_bought).reversed());
                    msj += "Los productos en el rango de mayor a menor son:\n";

                } else {
                    resultados.sort(Comparator.comparing(Product::getNumber_bought));
                    msj += "Las órdenes en el rango de precio de menor a mayor son:\n";
                }

                for (int i = 0; i < resultados.size(); i++) {
                    msj += " la orden es  " + resultados.get(i).getNumber_bought() + "  a nombre de "
                            + resultados.get(i).getName() + "\n";
                }
                break;
        }
    ...
    public String searchPrefProduct(String start, String end) {
        if (start == null || end == null) {
            throw new IllegalArgumentException("prefijos diferentes a nulos o a numeros");
        }
        StringBuilder result = new StringBuilder();
        ...
        int startIndex = search.binsearchabbPrefIzP(products, start);
        int endIndex = search.binsearchabbPrefDeP(products, end);
        if (startIndex < 0) {
            startIndex = -(startIndex + 1);
        }
        if (endIndex < 0) {
        ...
        for (Product orden : resultados) {
            result.append("orden a nombre de ").append(orden.getName()).append("\n");
        }
